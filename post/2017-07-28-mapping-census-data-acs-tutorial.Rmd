---
title: "Mapping Census Data (ACS) Tutorial"
author: "Gwendolyn Reynolds"
date: '2017-07-28'
slug: mapping-census-data-acs-tutorial
tags:
- R Markdown
- acs
- school districts
...
Leaflet is one of my favorite packages. The color choice brings me an unreasonable amount of joy, as do the slightly translucent city and street names. It's a gorgeous design that makes maps look sleek and modern. So when my colleague, Jon Zadra, decided to map out preschool aged children for a project we are working on in Oregon, I got really excited. He took ACS data and used leaflet to create a heat map of where preschool aged children were located along coastal Oregon. When I saw his maps, I knew I had to try it! And lucky me, there is a great, easy to follow, straight forward tutorial [here.](http://zevross.com/blog/2015/10/14/manipulating-and-mapping-us-census-data-in-r-using-the-acs-tigris-and-leaflet-packages-3/) 
...
```{r utah food stamp map set up, message=FALSE, warning=FALSE}
...
## The acs and tigris packages need the FIPS number for your geography. I found the FIPS number for every Utah County. You can google around to find these manually. 
...
## You will need to get an api key from the Census Website. You can request a key here: http://api.census.gov/data/key_signup.html. They will send you an email with an api key. The code below takes the key and sends it to the Census so you can access their tables.
...
```{r SCLO school district map, message=FALSE, warning=FALSE}
...
## If you don't have an api key, you will need to get an api key from the Census Website. You can request a key here: http://api.census.gov/data/key_signup.html. They will send you an email with an api key. The code below takes the key and sends it to the Census so you can access their tables.
